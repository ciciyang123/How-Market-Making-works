...
class ETFOptionChain:
    ...
    def __load_option_chain(self):
        option_class_dict = {'510050.SH': '50ETF', '159919.SZ': '300ETF_SZ',
                             '510300.SH': '300ETF_SH'}

        path = os.path.join("./exchange/", 'data', option_class_dict[self.underlying], 'static_file', self.end_date.replace('-', '') + ".csv")
        ...
        option_data["call_put"] = option_data["call_put"].map({'C': u'认购', 'P': u'认沽'})
    ...
    def strikes_expires(self):
        data = self.option_chain[['expiredate', 'strike_price', 'call_put', 'option_code']]
        data = DataFrame(data=data)
        data = data.rename(columns={'expiredate': 'expire', 'strike_price': 'strike', 'call_put': 'call_put'})
        return data
    ...
    def full_with_month(self, month: str):
        full = self.option_chain[self.option_chain['month'] == month]
        return full
    ...
    def option_code(self, strike: float = None, month: str = None, expire: float = None, option_type: str = None):
        x = self.option_chain
        if strike:
